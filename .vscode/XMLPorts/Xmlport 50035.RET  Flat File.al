xmlport 50035 "RET  Flat File"
{
        ...
        textelement(Root)
        {
            ...
            tableelement("Item Ledger Entry"; "Item Ledger Entry")
            {
                ...
                textelement(In_OrderDt1)
                {
                    ...
                    trigger OnBeforePassVariable()
                    begin
                        //In_OrderDt1 :=FORMAT("Item Ledger Entry"."Posting Date",0,'<Year4><Month,2><Day,2>');
                    end;
                }
                textelement(DOA1)
                {
                ...
                textelement(In_PostingDt1)
                {
                    ...
                    trigger OnBeforePassVariable()
                    begin
                        In_PostingDt1 := FORMAT("Item Ledger Entry"."Posting Date", 0, '<Year4><Month,2><Day,2>');
                    end;
                }
                textelement(DOA2)
                {
                ...
                begin
                    ...
                    IF RecVLE.FINDFIRST THEN BEGIN
                        IF RecPcrH.GET(RecVLE."Document No.") THEN
                            IF RecPcrH."DOA Transaction" THEN
                                DOA2 := FORMAT(RecPcrH."Posting Date", 0, '<Year4><Month,2><Day,2>');
                    END;
                end;
    ...
    begin
        Fileno := "no.series".GetNextNo('RET', TODAY, TRUE);
    end;
