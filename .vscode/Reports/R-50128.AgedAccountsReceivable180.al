report 50128 "Aged Accounts Receivable>180"         //nav id_14125500
{
    DefaultLayout = RDLC;
    RDLCLayout = './AgedAccountsReceivable180.rdl';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            dataitem("Cust. Ledger Entry"; "Cust. Ledger Entry")
            {
                ...
                begin
                    SETRANGE("Posting Date", EndingDate + 1, 99991231D); //12319999D
                end;
            }
        ...
        dataitem(CurrencyTotals; Integer)
        {
            ...
            begin
                ...
                IF TempCurrencyAmount.FINDSET(FALSE, FALSE) THEN
                    REPEAT
                        IF TempCurrencyAmount.Date <> 99991231D THEN  //12319999D
                            AgedCustLedgEntry[GetPeriodIndex(TempCurrencyAmount.Date)]."Remaining Amount" :=
                              TempCurrencyAmount.Amount
                        ELSE
                            AgedCustLedgEntry[8]."Remaining Amount" := TempCurrencyAmount.Amount; //AgedCustLedgEntry[6]/...[7]."Remaining Amount" := TempCurrencyAmount.Amount;win248
    ...
    var
        ...
        TotalCustLedgEntry: array[7] of Record 21;
        GrandTotalCustLedgEntry: array[7] of Record 21;
        AgedCustLedgEntry: array[8] of Record 21;
    ...
    begin
        ...
        IF AgingBy = AgingBy::"Due Date" THEN BEGIN
            PeriodEndDate[1] := 99991231D; //12319999D;
        ...
        FOR i := 2 TO ARRAYLEN(PeriodEndDate) DO BEGIN
            //WIN316++
            IF i = 5 THEN
                EVALUATE(PeriodLength2, '<-60D>')
            ELSE
                IF i = 6 THEN
                    EVALUATE(PeriodLength2, '<-180D>')
                ELSE
                    EVALUATE(PeriodLength2, '<-30D>');

            PeriodEndDate[i] := PeriodStartDate[i - 1] - 1;
            PeriodStartDate[i] := CALCDATE(PeriodLength2, PeriodEndDate[i] + 1);
            //MESSAGE('%3 - %1 - %2 ',PeriodEndDate[i],PeriodStartDate[i],i);
            //WIN316--
        END;
    ...
    begin
        ...
        END;
        TempCurrencyAmount."Currency Code" := CurrencyCode;
        TempCurrencyAmount.Date := 99991231D; //12319999D;
        IF TempCurrencyAmount.FIND THEN BEGIN
    ...
    begin
        ...
        ExcelBuf.CreateNewBook(Text011); //kj
        ExcelBuf.WriteSheet(Text011, COMPANYNAME, USERID); //kj
        ExcelBuf.OpenExcel(); //kj
