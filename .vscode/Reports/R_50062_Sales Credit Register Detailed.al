report 50105 "SalesCreditRegisterDetailed"
{
    DefaultLayout = RDLC;
    ApplicationArea = All;
    UsageCategory = ReportsAndAnalysis;
    RDLCLayout = './Sales Credit Register Detailed.rdl';
    ...
    dataset
    {
        dataitem("Sales Invoice Header"; "Sales Cr.Memo Header")
        {
            ...
            dataitem("Sales Invoice Line"; "Sales Cr.Memo Line")
            {
                ...
                column(Tax_SalesInvoiceLine; '')//"Sales Invoice Line"."Tax %"--CCIT
                {
                }
                column(TaxAmount_SalesInvoiceLine; '')//"Sales Invoice Line"."Tax Amount"--CCIT
                {
                }
                column(AmountIncludingTax_SalesInvoiceLine; "Sales Invoice Line".Amount + ABS(CGSTAmt) + ABS(SGSTAmt) + ABS(IGSTAmt))//"Sales Invoice Line"."Amount Including Tax"--CCIT
                {
                }
                column(Sno; Sno)
                {
            ...
            begin
                ...
                IF Payment_Method.GET("Sales Invoice Header"."Payment Terms Code") THEN;
    ...
    var
        ...
        BRANCH_NAME: Text;
        "Transporter Table": Record "Shipping Agent";//Need to check
